...
def process(thickness_si3n4, thickness_sio2):
...
if __name__ == "__main__":
    ...
    with Pool(processes=8) as pool:
        res = pool.starmap(process, thickness_combinations)

        for i, rgb in enumerate(res):
            # Plot rectangle on graph
            retangle = plt.Rectangle(
                (thickness_combinations[i][0], thickness_combinations[i][1]),
                thickness_step,
                thickness_step,
                color=rgb,
            )
            ...
            print(f"Plotting Progress: {i/len(res)*100:.2f}%")
